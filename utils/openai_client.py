...
class OpenAIClient:
        ...
        for attempt in range(config.OPENAI_MAX_RETRIES):
            try:
                ...
                if e.status_code in [400, 401, 403, 404]:
                    raise # Re-raise immediately for these errors
    ...
    async def generate_question(self, topic: str, difficulty: str) -> dict | None:
        ...
        try:
            ...
            response = await self._openai_call_with_retry(
                ...
                # model="gpt-3.5-turbo", # Ensure model is specified, or use default from helper
            ...
            if response and response.choices:
                ...
                try:
                    ...
                    if all(k in data for k in ["question", "intended_answer", "difficulty_assessment"]):
                        ...
                        logger.error(f"OpenAI response missing required keys: {content}")
    ...
    async def evaluate_answer(self, question: str, intended_answer: str, user_answer: str) -> dict | None:
        ...
        - "explanation": A string. If the status is "Partially correct", provide a brief explanation of what makes it partial (e.g., "The user mentioned X, but a full answer also includes Y."). If "Correct" or "Incorrect", this can be null or a very brief confirmation.
                ...
                try:
                    ...
                    if all(k in data for k in ["status", "explanation"]):
                        if data["status"] not in ["Correct", "Incorrect", "Partially correct"]:
                            logger.warning(f"OpenAI returned invalid status: {data['status']}")
                            data["status"] = "Incorrect" # Default to incorrect if status is weird
                        return data
                    else:
                        logger.error(f"OpenAI response missing required keys for eval: {content}")
                        return {"error": "OpenAI eval response missing required keys."}
                except json.JSONDecodeError as e:
                    logger.error(f"Failed to parse JSON from OpenAI eval response: {content}. Error: {e}")
